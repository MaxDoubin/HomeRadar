...
/// entry for `_http._tcp` in Info.plist (iOS's Local Network privacy
/// prompt), documented in `mobile/ios/README.md`.
@MainActor
final class ApplianceDiscovery: ObservableObject {
    ...
    /// results carry an unresolved Bonjour endpoint, not a host/port
    /// directly, so a short-lived `NWConnection` is used to resolve one.
            ...
            case .ready:
                if let remote = connection?.currentPath?.remoteEndpoint,
                   case let .hostPort(host, port) = remote {
                    ...
                    if !self.discovered.contains(where: { $0.host == candidate.host && $0.port == candidate.port }) {
